...
function SummaryCard({ title, value, subtitle, icon, color, soft, delay, t }) {
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: t.card, borderRadius: 16, border: `1px solid ${hovered ? color + '44' : t.border}`,
        padding: "22px 24px",
        boxShadow: hovered ? `0 8px 32px ${color}22` : t.shadow,
      ...
      <div style={{ fontSize: 11, color: t.textMuted, fontWeight: 500 }}>{subtitle}</div>
    </div>
  );
}

function BalanceTrend({ txns, t }) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
      <div style={{ overflowX: "auto", width: "100%" }}>
        <svg viewBox={`0 0 ${W} ${H + 30}`} style={{ width: "100%", minWidth: 300, height: "auto" }}>
          <defs>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginTop: 16 }}>
        {[
          { label: "Highest Balance", value: fmt(highestMonth.balance), sub: `${highestMonth.month} ${highestMonth.year}`, color: t.green,  icon: "↑" },
          { label: "Lowest Balance",  value: fmt(lowestMonth.balance),  sub: `${lowestMonth.month} ${lowestMonth.year}`,  color: t.orange, icon: "↓" },
          { label: "Average Balance", value: fmt(avgBalance),            sub: `Last ${monthlyData.length} Months`,         color: t.text,   icon: "~" },
        ].map((s, i) => (
          <div key={i} style={{ backgroundColor: t.surface, borderRadius: 10, padding: "10px 12px", border: `1px solid ${t.border}` }}>
...
function SpendingDonut({ txns, t }) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap", justifyContent: "center", flex: 1 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 5, flex: 1, minWidth: 200, width: "100%", overflow: "hidden" }}>
          {data.slice(0, 6).map((d, i) => (
            <div key={i}
              style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, padding: "3px 6px", borderRadius: 6, opacity: hoverCat && hoverCat !== d.cat ? 0.35 : 1, transition: "all 0.2s", cursor: "pointer", backgroundColor: hoverCat === d.cat ? (CAT_COLORS[d.cat] || t.textMuted) + '15' : 'transparent', width: "100%" }}
              onMouseEnter={() => setHoverCat(d.cat)} onMouseLeave={() => setHoverCat(null)}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginTop: 16 }}>
        {[
          { label: "Categories",    value: data.length,       sub: "Tracked",          color: t.text,   icon: "⊞" },
          { label: "Avg / Category", value: fmt(avgPerCat),    sub: "Per category",     color: t.green,  icon: "∅" },
          { label: "Top Category",   value: `${Math.round((maxCat.val/total)*100)}%`, sub: maxCat.cat, color: CAT_COLORS[maxCat.cat] || t.orange, icon: "★" },
        ].map((s, i) => (
          <div key={i} style={{ backgroundColor: t.surface, borderRadius: 10, padding: "10px 12px", border: `1px solid ${t.border}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
...
function AddModal({ onAdd, onClose, t, editTxn }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        backgroundColor: t.card, borderRadius: 20, padding: 28, width: "100%", maxWidth: 420,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <div style={{ flex: 1 }}><label style={labelStyle}>Amount (₹)</label><input style={inputStyle} type="number" value={form.amount} onChange={e => upd("amount", e.target.value)} placeholder="0" /></div>
...
function InsightsSection({ txns, t }) {
  const insights = useMemo(() => {
    ...
    const result = [];
    if (topCat) result.push({ title: "Highest Spending", value: topCat[0], sub: `${fmt(Math.round(topCat[1]))} total`, color: "#F030E4", icon: Icons.premiumFlame });
    ...
    if (prev && latest) {
      ...
      result.push({
        ...
        icon: pct > 0 ? Icons.premiumChartUp : Icons.premiumChartDown,
      });
    }
    ...
    result.push({ title: "Savings Rate", value: `${savingsRate}%`, sub: "of total income saved", color: savingsRate > 20 ? t.green : t.orange, icon: Icons.premiumMoneyBag });

    const avgTxn = Math.round(totalExpense / expenses.length);
    result.push({ title: "Avg. Transaction", value: fmt(avgTxn), sub: `across ${expenses.length} expenses`, color: t.text, icon: Icons.premiumMetricBar });
  ...
  return (
    <div style={{ padding: "0" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
        {insights.map((ins, i) => (
          <div key={i} style={{
            backgroundColor: t.cardAlt, borderRadius: 12, padding: "20px", display: "flex", flexDirection: "column",
...
export default function FinanceDashboard() {
  ...
  const cardWrap = (children, style = {}) => (
    <div style={{
      ...
      padding: "20px 22px", boxShadow: t.shadow, ...style,
    }}>{children}</div>
  );

  const sectionTitle = (text) => (
    <div style={{ fontSize: 16, fontWeight: 800, textTransform: "uppercase", letterSpacing: "1.5px", marginBottom: 20, display: "flex", alignItems: "center", gap: 10, color: t.text }}>
      {text}
  ...
  return (
    <div style={{
      backgroundColor: t.bg, minHeight: "100vh", display: "flex", fontFamily: "'Inter', sans-serif", opacity: mounted ? 1 : 0, transition: "opacity 0.5s ease"
    }}>
      ...
      <div className="app-container" style={{
        ...
        >
          ...
          <div style={{
            ...
            <div style={{
              ...
              {/* Role Select in Sidebar */}
              <div style={{ opacity: sidebarOpen ? 1 : 0, transition: "opacity 0.15s ease", width: "100%", overflow: "hidden" }}>
                <select 
                  value={role} 
                  onChange={e => setRole(e.target.value)} 
                  style={{ 
                    width: "100%", padding: "6px 8px", borderRadius: 8, 
                    border: `1px solid ${t.border}`, backgroundColor: t.cardAlt, 
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ position: "relative", flexShrink: 0, width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <div style={{
                    ...
                    backgroundImage: "url('https://api.dicebear.com/7.x/notionists/svg?seed=Bogdan&backgroundColor=1A1A1A')",
                    backgroundSize: "cover", border: `1px solid ${t.border}`, position: "absolute"
                  }} />
                  <div style={{
                    position: "absolute", top: -6, right: -6, width: 9, height: 9,
                    borderRadius: "50%", backgroundColor: t.green, border: `2px solid ${t.surface}`,
                  }} />
                </div>
                <div style={{ overflow: "hidden", opacity: sidebarOpen ? 1 : 0, transition: "opacity 0.15s ease", paddingLeft: 4 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: t.text, whiteSpace: "nowrap" }}>Bogdan Nikitin</div>
                  <div style={{ fontSize: 11, color: t.textMuted, whiteSpace: "nowrap" }}>{role === 'admin' ? 'System Administrator' : 'Project Viewer'}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mob-top-bar">
          <img src={isDark ? "/dark_theme_logo.png" : "/light_theme_logo.png"} alt="RayanFinTech" style={{ height: 28, objectFit: "contain" }} />
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {tabs.map(tab => {
            ...
            return (
              <div key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
                display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4, cursor: "pointer", color: isActive ? t.green : t.textMuted, width: "33%"
              }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", position: "relative" }}>
          ...
          <div ref={mainContentRef} className="main-content-area" style={{ flex: 1, overflowY: "auto", backgroundColor: t.bg }}>
            ...
            <div className="desk" style={{ marginBottom: 40, display: "flex", alignItems: "center", gap: 20 }}>
              <h1 style={{ fontFamily: "Oswald", fontSize: 52, fontWeight: 800, letterSpacing: 2, textTransform: "uppercase", margin: 0, color: t.text, lineHeight: 1 }}>
                {activeTab === "overview" ? "Dashboard" : activeTab === "transactions" ? "Transactions" : "Insights"}
              </h1>
            </div>
            ...
            {activeTab === "overview" && (
              <div style={{ animation: "fadeSlideIn 0.4s ease" }}>
                ...
                <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
                  <div>
                    <p style={{ margin: 0, fontSize: 14, color: t.textSec, fontWeight: 500 }}>Welcome back 👋 Here's your financial overview.</p>
                  </div>
                ...
                {/* Summary Cards */}
                <div className="sgrid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 24 }}>
                  <SummaryCard title="Total Balance" subtitle="Available funds" value={fmt(Math.round(totals.balance))} icon={Icons.wallet(t.green)} color={t.green} soft={t.greenSoft} delay={100} t={t} />
                  <SummaryCard title="Total Income" subtitle="This period" value={fmt(Math.round(totals.income))} icon={Icons.trendUp(t.green)} color={t.green} soft={t.greenSoft} delay={200} t={t} />
                  <SummaryCard title="Total Expenses" subtitle="This period" value={fmt(Math.round(totals.expense))} icon={Icons.trendDown(t.orange)} color={t.orange} soft={t.orangeSoft} delay={300} t={t} />
                </div>

                {/* Charts */}
                <div className="mgrid" style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 14, marginBottom: 20 }}>
                  {cardWrap(<>{sectionTitle("Balance Trend")}<BalanceTrend txns={txns} t={t} /></>, { display: "flex", flexDirection: "column" })}
                  {cardWrap(<>{sectionTitle("Spending Breakdown")}<SpendingDonut txns={txns} t={t} /></>, { display: "flex", flexDirection: "column" })}
                </div>
                ...
                {(() => {
                  ...
                  return (
                    <div style={{
                      ...
                      borderRadius: 14, backgroundColor: t.card, border: `1px solid ${isPositive ? t.green + '44' : t.orange + '44'}`,
                      borderLeft: `4px solid ${isPositive ? t.green : t.orange}`,
                      ...
                      <button onClick={() => setActiveTab("insights")} style={{
                        ...
                        backgroundColor: "transparent", color: isPositive ? t.green : t.orange, fontWeight: 700, fontSize: 11, cursor: "pointer", fontFamily: "Oswald", whiteSpace: "nowrap",
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                      ...
                      <div style={{ display: "flex", gap: 4 }}>
                        {["All", "income", "expense"].map(tp => pillBtn(filterType === tp, tp === "All" ? "All" : tp.charAt(0).toUpperCase() + tp.slice(1), () => setFilterType(tp)))}
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <>
                          ...
                          <tbody>
                            {filteredTxns.slice(0, 50).map((tx, i) => (
                              <tr key={tx.id} style={{ borderBottom: `1px solid ${t.border}`, transition: 'background 0.15s', animation: `fadeSlideIn 0.3s ${Math.min(i * 0.02, 0.5)}s both` }}
                                ...
                                <td style={{ padding: '12px', whiteSpace: 'nowrap', color: t.textSec, fontWeight: 500, fontVariantNumeric: 'tabular-nums' }}>{fmtDate(tx.date)}</td>
                                ...
                                <td style={{ padding: '12px', textAlign: 'right', fontWeight: 800, color: tx.type === 'income' ? t.green : t.orange, fontVariantNumeric: 'tabular-nums' }}>
                    ...
                    <div className='mob' style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {filteredTxns.slice(0, 50).map((tx, i) => {
                        ...
                        return (
                          <div key={tx.id} style={{ display: 'flex', alignItems: 'center', gap: 12, backgroundColor: t.card, borderRadius: 14, padding: '14px 16px', border: `1px solid ${t.border}`, animation: `fadeSlideIn 0.3s ${Math.min(i * 0.02, 0.4)}s both` }}>
                            <div style={{ width: 44, height: 44, borderRadius: 12, flexShrink: 0, backgroundColor: (isIncome ? t.green : (CAT_COLORS[tx.category] || t.orange)) + '18', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            {activeTab === "insights" && (
              <div style={{ animation: "fadeSlideIn 0.4s ease" }}>
                {cardWrap(<>{sectionTitle("Key Observations")}<InsightsSection txns={txns} t={t} /></>)}
    
                <div className="mgrid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginTop: 14 }}>
                  {cardWrap(<>{sectionTitle("Income vs Expenses")}<BalanceTrend txns={txns} t={t} /></>, { display: "flex", flexDirection: "column" })}
                  {cardWrap(<>{sectionTitle("Category Breakdown")}<SpendingDonut txns={txns} t={t} /></>, { display: "flex", flexDirection: "column" })}
                </div>
                      ...
                      <>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                          {months.map((m, i) => {
                            ...
                            return (
                              <div key={m} style={{ display: "flex", alignItems: "center", gap: 12, animation: `fadeSlideIn 0.4s ${i * 0.08}s both` }}>
                                <span style={{ width: 50, fontSize: 11, fontWeight: 700, color: t.textSec, textAlign: "right", flexShrink: 0 }}>{label}</span>
      ...
      {showModal && isAdmin && <AddModal onAdd={addOrUpdateTxn} onClose={() => { setShowModal(false); setEditTxn(null); }} t={t} editTxn={editTxn} />}
      </div>
    </div>
  );
}
